...
def generate_network(uz, ux):
    ...
    for i in range(len(uz)):
        for j in range(i + 1, len(uz)):
            ...
            logit = np.exp(PARAM['alpha0'] - PARAM['alpha1'] * np.sqrt(np.square(zi-zj))[0] - PARAM['alpha2'] * np.sqrt(np.square(xi-xj))[0]) * PARAM['network_density']
...
def main():
    ...
    x = np.random.uniform(0,1,size=PARAM['num_nodes'])
    ux = pd.DataFrame(x, columns=['x'])

    A = generate_network(uz, ux)
    ...
    # generate T0
    influence = cal_influence(A, np.array(y0_binary).T[0])   # (num_nodes, treat_dim)

    # generate y1
    # y0: (num_nodes, 1), motif_vec_0: (num_nodes, treat_dim), ux0.iloc[:,1:]: (num_nodes, feature_dim)
    y1 = pd.DataFrame(generate_next_Y(y0, influence, uz, set_columns=['y1'], seed=seed))
    influence = pd.DataFrame(influence, columns=['influence_0'])

    dt_train = pd.concat([uz, y0, y1, influence, ux], axis=1)
...
PARAM = {
    ...
    # 1. net_param
    'alpha0': 0,
    'alpha1': 1.5,  # only record alpha_1 (contribution of unobserved Z)
    'alpha2': 1.5,
