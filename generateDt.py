...
def generate_network(uz):
    ...
    for i in range(len(uz)):
        for j in range(i + 1, len(uz)):
            ...
            logit = np.exp(PARAM['alpha0'] - PARAM['alpha1'] * np.linalg.norm([zi, zj])) * PARAM['network_density']
...
def main():
    ...
    # generate y0
    y0 = pd.DataFrame(np.random.normal(0, 0.5, size=PARAM['num_nodes']), columns=['y0'])  # y(t-1)
    y0_binary = y0.copy()
    y0_binary[y0_binary<0]=0
    y0_binary[y0_binary>0]=1

    # generate T0
    influence_0 = cal_influence(A, np.array(y0_binary).T[0], uz, uz_pi)   # (num_nodes, treat_dim)

    # generate y1
    # y0: (num_nodes, 1), motif_vec_0: (num_nodes, treat_dim), ux0.iloc[:,1:]: (num_nodes, feature_dim)
    y1 = pd.DataFrame(generate_next_Y(y0, influence_0, uz, set_columns=['y1'], seed=seed))
    influence_0 = pd.DataFrame(influence_0, columns=['influence_0'])

    dt_train = pd.concat([uz, y0, y1, influence_0], axis=1)
