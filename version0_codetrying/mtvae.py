...
class Model(PyroModule):
    ...
    def __init__(self, tau_weight):
        ...
        self.t_nn = MultivariateBetaNet([self.latent_dim] + [self.hidden_dim]*self.num_layers,
                                        self.treat_dim)
        # f3
        self.y_nn = NormalMeanNet([self.latent_dim+self.tau_dim] + [self.hidden_dim]*self.num_layers)
        # # f4
    ...
    def forward(self, xcon, t=None, y=None, size=None):
        if size is None:
            size = xcon.size(0)
        # TODO: check subsample
        with pyro.plate("data", size, subsample=xcon):
            z = pyro.sample("z", self.z_dist())                     # z: (batch_size, latend_dim)
            xcon = pyro.sample("x", self.xcon_dist(z), obs=xcon) # xcon: (batch_size, xcon_dim)
            t = pyro.sample("t", self.t_dist(z), obs=t)             # t: (batch_size, treat_dim)
            y = pyro.sample("y", self.y_dist(t, z), obs=y)          # y: (batch_size)
            return y

    def y_mean(self, xcon, t=None):
        # TODO: check x.size(0)
    ...
    def y_dist(self, t, z):
        t = t.float()
        tau = self.tau_weight(t)
        tz = torch.cat((tau, z), dim=1).float()
        (mu_yp,) = self.y_nn(tz)
...
class Guide(PyroModule):
    ...
    def __init__(self, tau_weight):
        ...
        self.y_nn = NormalMeanNet([self.feature_dim + self.treat_dim,
                                   self.hidden_dim])
        # g5
        self.z_nn = DiagNormalNet([self.feature_dim + self.tau_dim + 1,
            self.hidden_dim,
            self.latent_dim])
        self.tau_weight = tau_weight

    def forward(self, x_con, t=None, y=None, size=None):
        if x_con.dim()==1:
    ...
    def z_dist(self, y, t, x):
        t = t.float()
        tau = self.tau_weight(t)
        ytx = torch.cat((y.unsqueeze(1),tau,x), dim=1).float()  # concat, ytx: (batch_size, feature_dim + treatment_dim + 1)
...
class MTVAE(nn.Module):
    def __init__(self):
        ...
        self.learnable_t_weight = torch.nn.Linear(PARAM['treat_dim'],PARAM['tau_dim'])

        self.model = Model(self.learnable_t_weight)
        self.guide = Guide(self.learnable_t_weight)

    def fit(self, x_con, t, y):
        ...
        dataset = TensorDataset(x_con, t, y)
        ...
        for epoch in range(num_epochs):
            for x_con, t, y in dataloader:
                loss = svi.step(x_con, t, y, size=len(dataset)) / len(dataset)
    ...
    def potential_y(self, x_con, t):
        ...
        d=torch.cat((x_con, t),dim=-1) # d: (len(data), feature_dim+treat_dim)
        dataloader = [d]
        ...
        for d in dataloader:
            ...
            with pyro.plate("num_particles", mc_samples, dim=-2):
                with poutine.trace() as tr, poutine.block(hide=hide_list):
                    self.guide(d[:,0])    # xbi, xcon
                with poutine.do(data=dict(t=torch.tensor(d[:,1:]))):
                    y_hat = poutine.replay(self.model.y_mean, tr.trace)(d[:,0])
...
def get_dt(path):
    dt = pd.read_csv(path)
    x_con = torch.tensor(dt['x0']).float()

    if x_con.dim()==1:
        x_con = x_con.unsqueeze(1)
    t = torch.tensor(np.array(dt.loc[:,'t00':'t07']))
...
def get_testdt(path):
    ...
    if x_con.dim()==1:
        x_con = x_con.unsqueeze(1)
    t = torch.tensor(np.array(dt.loc[:,'t10':'t17']))
    y = torch.tensor(dt['y2'])
    return x_con, t, y
...
def main():
    ...
    mtvae = MTVAE()
    mtvae.fit(x_con, t, y)
    z = mtvae.check_z(x_con, t)
    pd.DataFrame(np.mean(z.detach().numpy(),axis=0)).to_csv('save_emb/mtvae_'+str(PARAM['num_nodes'])+'/'+PARAM['graph']+'/mtvae_dim_' + str(PARAM['latent_dim']) + '_Id'+str(net_id)+'.csv',
                                                  index=False)

    # Evaluate.
    x_con, t, y = get_dt(PARAM['data'])
    est_y = mtvae.potential_y(x_con, t)    # est_y: (len(data))
...
if __name__ == "__main__":
    ...
    graph = 'test'
    set_seed(SEED)  # model seed
    for net_id in range(11,111):
        PARAM = {
            'description': "Multiple Treatment Causal Effect Variational Autoencoder",
            # data
            'data': 'data/gendt/'+graph+'/gendt_'+str(net_id)+'.csv',
            'graph': graph,
            'num_nodes': 100,
            'xbi_dim': 0,
            'xcon_dim': 1,
        ...
        print("training mtvae with data id ", net_id)
        main()