...
class PoissonMF(BaseEstimator, TransformerMixin):
    ...
    def _update_theta(self, X):
        ...
        outcome_term = np.multiply(np.exp(self.Elogt), np.dot(ratio_obs, np.exp(self.Elogb).T))
...
data = 'A_'+str(PARAM['num_nodes'])+'_'+str(PARAM['num_items'])

if __name__ == '__main__':
    N, K = PARAM['num_nodes'], PARAM['num_items']
    latent = PARAM['z_dim']
    # Z = stats.gamma.rvs(0.5, scale=0.1, size=(N, latent))
    # Theta = stats.gamma.rvs(0.5, scale=0.1, size=(K, latent))
    for i in range(11,111):
        dir_dt = pd.read_csv('data/genRec/' + data + '/genRec_'+str(i)+'.csv')
        X = dir_dt['y0']
        X.binary = X.copy()
        X[X>0]=1
        X[X<0]=0
        ...
        pmf = PoissonMF(n_components=latent, verbose=True, max_iter=1000)
        pmf.fit(X)
        ...
        if save_emb == True:
            emb_path = 'save_emb/pfm/' + data + '_zdim_' + str(PARAM['z_dim'])
            if not os.path.isdir(emb_path):
                os.makedirs(emb_path)
            pd.DataFrame(pmf.Et).to_csv(emb_path + '/emb_' + str(i) + '.csv', index=False)
